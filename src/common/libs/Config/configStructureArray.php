...
$configStructureArray = [
  ...
  "SMTP_PORT" => [
    "form" => [
      ...
      "options" => [],
      "verifyMatch" => function ($value, $options) {
        return ["valid" => filter_var($value, FILTER_VALIDATE_INT), "value" => $value, "error" => filter_var($value, FILTER_VALIDATE_INT) ? '' : "Invalid port"];
      }
    ],
    "specialRequest" => true,
    "default" => 587,
  ...
  "SMTP_SECURE" => [
    "form" => [
      ...
      "name" => "SMTP Secure",
      "group" => "Email",
      "description" => "The SMTP security to use to send emails",
      "required" => false,
      "maxlength" => 3,
      "minlength" => 3,
      "options" => ["ssl", "tls"],
      "verifyMatch" => function ($value, $options) {
        return ["valid" => in_array($value, $options), "value" => $value, "error" => in_array($value, $options) ? '' : "Invalid security"];
      }
    ],
    "specialRequest" => true,
    "default" => "tls",
